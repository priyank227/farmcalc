...
import { HardHat, Trash2, Plus, Pencil, ChevronDown, Calendar } from 'lucide-react';
      ...
      <div className="p-4 space-y-6 pb-20">
        {expenses.length > 0 && (
          <div className="bg-green-50 rounded-3xl p-5 flex items-center justify-between">
            <div>
              <p className="text-gray-600 text-sm font-medium mb-1">{t('totalMajuriGiven')}</p>
            ...
            <div className="w-12 h-12 bg-green-200/50 rounded-2xl flex items-center justify-center">
              <HardHat className="w-6 h-6 text-[#166534]" />
            </div>
          </div>
        )}
        ...
        {role === 'worker' ? (
          <div className="bg-blue-50 border border-blue-100 rounded-3xl p-5 text-center">
            <p className="text-blue-600 text-sm font-medium">Read-Only Mode: You cannot add or edit majuri.</p>
          </div>
        ) : (
          <div>
            <h2 className="text-gray-900 font-bold mb-3">{t('addMajuri')}</h2>
            {workers.length === 0 ? (
              <div className="bg-yellow-50 border border-yellow-100 rounded-2xl p-4 text-center">
                <p className="text-yellow-600 text-sm font-medium">{t('addWorkersFirst')}</p>
              </div>
            ) : (
        ...
        <div>
          ...
          {loading ? (
            ...
            <div className="bg-white border border-gray-200 rounded-3xl p-10 text-center text-gray-400 font-medium">{t('noMajuriYet')}</div>
          ) : (
      ...
      {editModal && (
        <div className="fixed inset-0 bg-gray-900/40 backdrop-blur-sm flex items-end justify-center z-50 p-4 pb-6">
          <div className="bg-white border border-gray-100 rounded-3xl p-6 w-full max-w-sm shadow-2xl">
            <div className="flex items-center gap-2 mb-1"><Pencil className="w-5 h-5 text-[#166534]" /><h2 className="text-xl font-bold text-gray-900">{t('editMajuri')}</h2></div>
