...
export default function PesticidePage() {
  ...
  const loadData = useCallback(async (force = false) => {
    ...
    if (!force && cached) { setExpenses(cached); return; }
    setLoading(true);
    try {
  ...
  return (
    <div className="min-h-screen bg-gray-950 flex flex-col pb-10">
      <PageHeader title="Farm Expenses" icon={Bug} iconBg="bg-orange-500/30" iconColor="text-orange-400" />
      <div className="p-4 space-y-4">
        {expenses.length > 0 && (
          <div className="bg-gradient-to-br from-orange-500/20 to-amber-600/20 border border-orange-500/20 rounded-3xl p-5">
            <p className="text-orange-300 text-sm font-medium mb-1">Total Farm Expenses</p>
            <p className="text-3xl font-black text-white">₹{total.toLocaleString('en-IN')}</p>
          </div>
        )}
        <div className="bg-white/5 border border-white/10 rounded-3xl p-5">
          <h2 className="text-white font-bold mb-4 flex items-center gap-2"><Plus className="w-4 h-4 text-orange-400" />Add Expense</h2>
          <form onSubmit={handleAdd} className="space-y-3">
            <input placeholder="Expense name (e.g. Urea)" value={name} onChange={e => setName(e.target.value)} className="w-full px-4 py-3 rounded-2xl bg-white/10 border border-white/20 text-white placeholder-white/30 outline-none focus:border-orange-400/60" required />
            <input type="number" placeholder="Amount (₹)" min="1" value={amount} onChange={e => setAmount(e.target.value)} className="w-full px-4 py-3 rounded-2xl bg-white/10 border border-white/20 text-white placeholder-white/30 outline-none focus:border-orange-400/60" required />
            ...
            <input placeholder="Note / Bill no. (optional)" value={comment} onChange={e => setComment(e.target.value)} className="w-full px-4 py-3 rounded-2xl bg-white/10 border border-white/20 text-white placeholder-white/30 outline-none focus:border-orange-400/60" />
            <button type="submit" disabled={submitting} className="w-full py-3.5 rounded-2xl bg-gradient-to-r from-orange-500 to-amber-600 text-white font-bold shadow-lg shadow-orange-500/25 disabled:opacity-50 active:scale-[0.98] transition-all">{submitting ? 'Saving...' : 'Add Expense'}</button>
          </form>
        </div>
        <h3 className="text-gray-400 font-semibold px-1 text-sm uppercase tracking-wider">All Expenses</h3>
        {loading ? (
          <div className="space-y-3">{[1,2].map(i => <div key={i} className="h-20 bg-white/5 rounded-3xl animate-pulse" />)}</div>
        ) : expenses.length === 0 ? (
          <div className="bg-white/5 border border-white/10 rounded-3xl p-10 text-center text-gray-500">No expenses recorded yet</div>
        ) : (
      ...
      {editModal && (
        <div className="fixed inset-0 bg-black/70 backdrop-blur-sm flex items-end justify-center z-50 p-4 pb-6">
          <div className="bg-gray-900 border border-white/10 rounded-3xl p-6 w-full max-w-sm shadow-2xl">
            <div className="flex items-center gap-2 mb-1"><Pencil className="w-4 h-4 text-orange-400" /><h2 className="text-xl font-bold text-white">Edit Expense</h2></div>
            <p className="text-gray-400 text-sm mb-5">Update expense details</p>
            <form onSubmit={handleEdit} className="space-y-3">
              <input placeholder="Expense name" value={editName} onChange={e => setEditName(e.target.value)} className="w-full px-4 py-3 rounded-2xl bg-white/10 border border-white/20 text-white placeholder-white/30 outline-none focus:border-orange-400/60" required autoFocus />
              <input type="number" placeholder="Amount (₹)" min="1" value={editAmount} onChange={e => setEditAmount(e.target.value)} className="w-full px-4 py-3 rounded-2xl bg-white/10 border border-white/20 text-white placeholder-white/30 outline-none focus:border-orange-400/60" required />
              ...
              <input placeholder="Note (optional)" value={editComment} onChange={e => setEditComment(e.target.value)} className="w-full px-4 py-3 rounded-2xl bg-white/10 border border-white/20 text-white placeholder-white/30 outline-none focus:border-orange-400/60" />
              <div className="flex gap-3 pt-2">
                ...
                <button type="submit" disabled={editing} className="flex-1 py-3.5 rounded-2xl bg-gradient-to-r from-orange-500 to-amber-600 text-white font-bold disabled:opacity-50">{editing ? 'Saving...' : 'Save'}</button>
              </div>
            </form>
          </div>
        </div>
      ...
      {deleteModal && (
        <div className="fixed inset-0 bg-black/70 backdrop-blur-sm flex items-end justify-center z-50 p-4 pb-6">
          <div className="bg-gray-900 border border-white/10 rounded-3xl p-6 w-full max-w-sm shadow-2xl">
            <h2 className="text-xl font-bold text-white mb-2">Delete Expense?</h2>
            <p className="text-gray-400 text-sm mb-5">₹{deleteModal.amount} · {deleteModal.name}. Enter PIN.</p>
